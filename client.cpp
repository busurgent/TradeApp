    ...
    try
    {
        ...
        while (true)
        {
            ...
            switch (menu_option_num)
            {
                ...
                {
                    std::string deal;
                    std::cout << "Input terms" << std::endl; 
                    std::cin >> deal; // Ввводим в виде "объем:цена:операция"
                    SendMessage(s, my_id, Requests::Trading, deal);
                    std::cout << ReadMessage(s);
                    break;
                }
                case 4:
                {
                    SendMessage(s, my_id, Requests::Status, "");
                    std::cout << ReadMessage(s);
                    break;
                }
                default:
